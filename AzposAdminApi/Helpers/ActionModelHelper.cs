...
namespace AzposAdminApi.Helpers
{
    ...
    {
        public T AssignCreateModel<T>(T model, string tableName, string userId, string? tenantId) where T : new()
        {
            Type type = typeof(T);
            ...
            PropertyInfo? profileId = props.FirstOrDefault(x => x.Name == "TenantId");
            if (profileId != null && !string.IsNullOrEmpty(tenantId))
            {
                profileId.SetValue(model, tenantId, null);
            }
        ...
        public T AssignCreateWithExistingId<T>(T model, string tableName, string existingId, string userId, string tenantId) where T : new()
        {
            Type type = typeof(T);
            PropertyInfo[] props = type.GetProperties(BindingFlags.Public | BindingFlags.Instance);
            ...
            if (modelId != null)
            {
                modelId.SetValue(model, existingId, null);
            }
